...
# Xing API로 데이터를 수신. 117{num} 포트로 데이터를 PUSH
# PUSH한 데이터는 split_subscriber에서 사용
...
print(f"kospi_codes: {len(kospi_codes)}. kosdaq_codes: {len(kosdaq_codes)}.")
...
class Main():
    ...
    def __init__(self):
        ...
        ## 나의 체결 데이터 요청
        rq_id = "SC1"
        stockDataManager1.create_instance(rq_id=rq_id, XAReal_class=XAReal_KOR, port=data_port, zmq_context=zmq_context)
